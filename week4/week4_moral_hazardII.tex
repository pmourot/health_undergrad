...
\begin{frame}
  ...
  % BIG TITLE
  {\fontsize{30}{32}\selectfont\bfseries\textcolor{maroon}{Moral Hazard - Part II}}\\[-0.25cm]
...
%---------------------------------------------------------------------
\input{sections/moral_hazardII.tex}
%---------------------------------------------------------------------
...
%---------------------------------------------------------------------
\begin{frame}{Intent to treat (ITT)}
  \hypertarget{ohie:ITT}{}
  ...
  \vspace{-3mm}
  \begin{align*}
      y_{i} = \pi_0 + \pi_1 \text{INSURANCE}_i + X_{i} \pi_2 + V_{i} \pi_3 + v_{i}
  \end{align*}
  
  First stage
  \vspace{-3mm}
  \begin{align*}
      \text{INSURANCE}_{i} = \delta_0 + \delta_1 \text{LOTTERY}_i + X_{i} \delta_2 + V_{i} \delta_3 + \mu_{i}
  \end{align*}
    \vspace{-5mm}
    \hyperlink{ohie:table}{\beamerreturnbutton{Back}}
\end{frame}
%---------------------------------------------------------------------
\begin{frame}{Price shopping: details}
  \hypertarget{BCHK:PS}{}

  \newbold{Price shopping} per procedure $m$
  \begin{align*}
      \Delta PS_{m,t+1,t} = \frac{P_{m,J^m,t+1} C_{m,J^m,t+1} - P_{m,J^m,t+1} C_{m,J^m,t}}{P_{m,J^m,t+1} C_{m,J^m,t}}
  \end{align*}
  where
  \begin{itemize}
      \item $C_{m,J^m,.}$ are vectors of provider choices 
      \item $P_{m,J^m,.}$ are vectors of prices 
      \item both across the set of providers for procedure $m$ $J^m$
  \end{itemize}
  \vspace{3mm}

  \newbold{Overall price shopping effect}
  \vspace{-2mm}
  \begin{align*}
      \Delta PS_{t+1,t} = \sum_{m=1}^{M} \frac{Y_{m,t}}{Y_t} \Delta PS_{m,t+1,t}
  \end{align*}   
  where $Y_{m,t}$ is the total procedure spending in $t$ 
  %Holds the share of spending at t

  %That's the extent to which consumers substitutes to higher or lower priced providers CONDITIONAL ON DOING PROCEDURE

  \vspace{-3mm}
  \hyperlink{BCHK:decomposition}{\beamerreturnbutton{Back}}
\end{frame}
%---------------------------------------------------------------------
%\input{sections/appendix_week1_selection_markets.tex}
%%---------------------------------------------------------------------

%\bibliographystyle{../bib/aea}
%\nobibliography{../bib/doctors}
